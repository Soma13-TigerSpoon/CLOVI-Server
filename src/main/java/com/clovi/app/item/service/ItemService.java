...
public class ItemService {
  ...
  public ItemResponse findItemById(Long itemId) {
      ...
      Item item = itemRepository.findByIdAndDeletedIsFalse(itemId).orElseThrow(() -> new ResourceNotFoundException("domain",itemId));
  ...
  public Long update(ItemUpdateRequest itemUpdateRequest, Long itemId, Member member) {
   Long userId = member.getId();
   Item findItem = itemRepository.findByIdAndDeletedIsFalse(itemId).orElseThrow(() -> new ResourceNotFoundException("domain", itemId));
