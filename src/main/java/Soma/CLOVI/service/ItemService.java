...
public class ItemService {
    ...
    public Long save(TimeItemRequestDto timeItemRequestDto) {
        ...
        Model model = modelRepository.findById(timeItemRequestDto.getModelId()).orElseThrow(() -> new IllegalArgumentException("해당 모델이 없습니다. id=" + timeItemRequestDto.getModelId()));
        Video video = videoRepository.findById(timeItemRequestDto.getVideoId()).orElseThrow(() -> new IllegalArgumentException("해당 비디오가 없습니다. id=" + timeItemRequestDto.getVideoId()));

        Long capturePoint = StringTimeToLong(timeItemRequestDto.getCapturePoint());
        TimeFrame timeFrame = timeFrameRepository.findByCapturePointBetween(capturePoint-2,capturePoint+2).orElse(
